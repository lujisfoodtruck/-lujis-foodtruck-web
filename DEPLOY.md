...
- **Caché del navegador tras cada actualización**: `index.html`, `menu.html` y `admin/index.html` referencian `style.css`, `config.js`, `i18n.js`, `main.js`/`menu.js`/`admin.js` con un `?v=2` al final (ej. `assets/js/admin.js?v=2`). Esto existe para que el navegador de quien ya visitó el sitio antes baje la versión nueva del archivo en vez de quedarse con una vieja en caché. **Cada vez que se suba una actualización que toque alguno de esos archivos, hay que subir el número (`?v=3`, `?v=4`, ...) en las 3 páginas HTML** — si no, algunos visitantes van a seguir viendo la versión anterior del sitio/admin hasta que el caché expire solo.
